...
It is fenced and delimiter-escaped like every other data block, for consistency rather than for defence: unlike file content or a chat message, these values are server-originated. They are read from `TestCase.automationKey` rows the platform itself wrote during ingestion, never typed by a user in this request. Escaping them still matters because their *origin* is a JUnit report from the connected repository, so they are untrusted at rest even though nothing in the request path can change them. Naming the targets does not grant the model new authority either — the response is still schema-constrained, re-validated with Zod, and matched back by exact `automationKey`, so a model that invents a key simply fails to match and leaves that target in the manual-review fallback. `EXTRACTION_PROMPT_VERSION` moved to `extraction-v4` with this block.

### Observations and the suite summary

`extraction-v5` asks for two more pieces of model output: up to five advisory `observations` per case, and a `suite` title and description for file-level jobs. Both are text the model wrote after reading an untrusted file, so they get the same treatment as a chat reply: bounded in Zod (five items of 200 characters; 80 and 300 characters), re-validated before persistence, and rendered as text, never as HTML. A hijacked file cannot smuggle more than a few short strings through them, and none of it reaches an official case or suite without a person approving it. The prompt tells the model observations are commentary and forbids proposing code, which keeps the platform on the documentation side of the thesis boundary even when the model is inclined to help.
